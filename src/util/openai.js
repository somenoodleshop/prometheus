...
const session = (query, systemPrompt = defaultSystemPrompt) => {
...
const query = messages => request.post(url, body(defaultModel, [{ role: 'system', content: defaultSystemPrompt }, ...messages]), authorization(OPENAI_TOKEN))
  .then(({ choices = [] }) => {
    const [{ message = '' }] = choices
